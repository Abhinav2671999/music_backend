...
router.get("/login", async (req, res) => {
    ...
    try {
        ...
        if (!decodeValue) {
            return res.status(505).json({ message: "un authroized" })
        } else {

            // checking user exixt already
...
router.put("/updateRole/:userId", async (req, res) => {
    ...
    try {
        const restult = await user.findOneAndUpdate(filter, { role: role });

        res.status(200).send({ user: restult });
    } catch (error) {
        res.status(400).send({ success: false, msg: error });
    }
});
